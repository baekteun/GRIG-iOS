    ...
    var description: NSMutableAttributedString {
        ...
        case .purpose:
            return NSMutableAttributedString(
                attributedString: About.parser.parse("GRIG는 광주소프트웨어마이스터고등학교 학생들의 Github 활동을 활성화하기 위해서 만들어졌습니다.")
            )
        case .funtion:
            return NSMutableAttributedString(
                attributedString: About.parser.parse("""
...
""")
        ...
        case .privacy:
            return NSMutableAttributedString(
                attributedString: About.parser.parse("GRIG OpenAPI 에 본인을 등록하실 경우, Github Profile 사진 활용 및 기타 유저 리스트에 필요한 사용자 정보 활용에 동의한 것으로 간주됩니다.")
            )
        }
    }
}
